...
static struct platform_driver pd // TODO: INIT THE STUFF
{
  { .compatible = "IDK WHAT GOES HERE YET", },
  {}
};
...
static int audio_init(void) 
{
  ...
  pr_info("Major Number for Audio: %d\n\r", major_num);
  ...
  the_class = create_class(THIS_MODULE, MODULE_NAME); // TODO: first paramter might be wrong
  ...
  pr_info("Platform driver number: %d\n\r", reg_platform_driver);
...
{   
  ...
  struct resource* mem_resource = request_mem_region(0, 0, MODULE_NAME); // TODO: check 1st and 2nd params

  // Get a (virtual memory) pointer to the device -- ioremap
  // 1st param - physical address
  // 2nd param - size in bytes
  ioremap(audio_device.phys.addr, 0); // TODO: find param 2
...
static int audio_remove(struct platform_device * pdev) 
{
  ...
  iounmap(audio_device.virt_addr);
